...
This model is better than fixed effects.
...
model_fit = lmer(trans_density ~ logsydist*logparradist*yearsince2000
                 + logsydist*logparradist*year2020
                 +  logsydist*logparradist*year2021
                 + (1|name),data=dataPop)
...
Expansion: The Model Summary table

The following code returns the R-squared values for the above mixed-effects model.
The package `MuMIn` gives this information, install version of R >= 4.2.0 beforehand!

```{r}
#library("MuMIn") # needs R > 4.2.0
r.squaredGLMM(model_fit)
```

Can we obtain AIC, BIC next?
