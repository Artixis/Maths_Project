...
# Linear Models
We'll start by graphing our transform variables of density and Sydney distance 
...
pop density ~ year*(distance_to_sydney) + (year==2020)* (distance_to_sydney) ) + (year==2021)* (distance_to_sydney)
...
```{r}
email_mod = lm(trans_density ~ yearsince2000*logsydist + (yearsince2000 = 2020)*logsydist, data = dataPop)
summary(email_mod)
```
