...
```{r, warning=FALSE,results='hide',collapse=TRUE,include=FALSE}
x <- c("ggmap", "rgeos", "maptools", "dplyr", "tidyr", "sf", "MASS", "rgeos", "gmt", "lme4", "lmerTest", "ggplot2", "sf", "tidyverse", "rgdal", "tmap", "gganimate")
lapply(x, library, character.only = TRUE)
```

Load in the population data set:
```{r}
dataPop = read.csv("C:/Users/viquy/Documents/GitHub/Maths_Project/R Scripts/Data Used/populationLong.csv")
```
...
```{r}
syd <- st_read("C:/Users/viquy/Documents/GitHub/Maths_Project/R Scripts/Data Used/SA2_2016_AUST.shp")
...
Create variables within the shape file sydist and parradist. These are the calculated distance of each SA2 from Sydney and Parramatta respectively.

Log these values to normalize them.

Insert these calculated values into the population data frame by matching the two files by SA2 name.
```{r}
...
```{r}
pop_lm = lm(trans_density ~ sydist, data = dataPop)
summary(pop_lm)
```
...
```{r}
pop_mlr = lm(trans_density ~ sydist + yearsince2000, data = dataPop)
summary(pop_mlr)
```
