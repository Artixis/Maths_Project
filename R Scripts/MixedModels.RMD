...
```{r}
# Let's try a very simpple random intercepts model.
...
```
```{r}
syd$coef <- random_coef$`(Intercept)`[match(unique(syd$SA2_NAME16),row.names(random_coef))]


qtm(shp = syd, fill = c("coef"), fill.palette = "Blues", title = "NSW Distance Map")
```
```{r}
